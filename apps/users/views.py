...
from apps.utils.Yupian import send_single_sms
from MxOnline_Class.settings import yp_apikey, REDIS_HOST, REDIS_PORT
...
class ChangePwdView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if pwd_form.is_valid():
            # pwd1 = pwd_form.cleaned_data['pwd1']
...
class DynamicLoginView(View):  # 手机动态登录
    def get(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            return HttpResponseRedirect(reverse("index"))
        next = request.GET.get("next", "")
...
class LoginView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if login_form.is_valid():
            ...
            # 表单验证
            # UserProfile.objects.create_user(username=user_name, email="whatever@some.com", password=password)
            #
            # user = UserProfile.objects.get(username=user_name)
            # pwd = user.password
            # flag = check_password(password, pwd)
            ...
            if user:
                ...
                # return HttpResponseRedirect('登录失败')
        else:
            return render(request, 'login.html', {"login_form": login_form,
                                                  "banners": banners,
                                                  })
